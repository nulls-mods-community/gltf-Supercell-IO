...
class OdinPackedReader(OdinAnimationReader):
    def __init__(self, gltf: glTFImporter, animation: dict):
        ...
        self.normalized_transform_data = BinaryData.decode_accessor(
            gltf, self.descriptor.get("dataAccessor")
        )

        # Base transform values
        self.node_base_data = BinaryData.decode_accessor(
            gltf, self.descriptor.get("nodeAccessor")
        )
        ...
        for frame_index in range(frame_count):
            for i in range(TRANSLATION_CHANNELS):
                ...
                if flags.has_translation:
                    transform = (
                        float(nTranslation[i][frame_index]) * translation_multiplier
                    )
                    value += transform
